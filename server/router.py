...
def build_live_graph(G_base: nx.MultiDiGraph,
                     ...
                     incident: dict | None = None) -> nx.MultiDiGraph:
    """
    ...
    """
    global _LIVE_EDGE_DATA
    _LIVE_EDGE_DATA = {}

    # Seed every edge with a baseline BPR weight (30% of capacity).
    ...
    # Batch compute nearest edges for any un-cached segments to avoid 800x O(E) spatial lookups
    uncached = []
    mid_lngs = []
    mid_lats = []
    ...
    # Apply live CSV per-segment overrides using the fast cache
    for _, row in frame_df.iterrows():
        seg_id = str(row.get("seg_id", ""))
    ...
    # Forcibly block the manually clicked / active incident segment
    if incident and incident.get("seg_id"):
        inc_seg_id = str(incident["seg_id"])
    ...
    try:
        ...
        for path in islice(paths_iter, k * 3):
            ...
            for u, v in zip(path[:-1], path[1:]):
                ...
                cap  = CAPACITY_MAP.get(edge_data.get("highway", "residential"), 500)
